...
public class MedalStat {
    ...
    public static class Mapper1 extends MapReduceBase implements Mapper<LongWritable, Text, Text, IntWritable> {
        ...
        public void map(LongWritable key, Text value, OutputCollector<Text, IntWritable> output, Reporter reporter) throws IOException {
            ...
            String sport = csvLine[12];
            String medal = csvLine[14];
            if ("\"Name\"".equals(name))
                return;
            int year = Integer.parseInt(csvLine[9]);
            if (year < 1996)
                return;
//            if (!"\"Gymnastics\"".equals(sport))
//                return;
            if ("NA".equals(medal))
                return;
            word.set(noc);
    ...
    public static void main(String[] args) throws Exception {
        // job1，负责count
        JobConf job1 = new JobConf(MedalStat.class);
        job1.setJobName("Test2-job1-count");

        job1.setOutputKeyClass(Text.class);
        job1.setOutputValueClass(IntWritable.class);

        job1.setMapperClass(MedalStat.Mapper1.class);
        job1.setCombinerClass(MedalStat.Reducer1.class);
        job1.setReducerClass(MedalStat.Reducer1.class);
        ...
        // job2，负责sort
        JobConf job2 = new JobConf(MedalStat.class);
        ...
        job2.setMapperClass(MedalStat.Mapper2.class);
//        job2.setCombinerClass(Test2.Reducer2.class);
        job2.setReducerClass(MedalStat.Reducer2.class);
